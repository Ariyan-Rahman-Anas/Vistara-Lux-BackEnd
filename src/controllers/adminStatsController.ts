...
export const getDashboardStats = async (
    ...
    try {
        let adminDashboardStats
        const key = "admin-dashboard-stats"
        if (dataCaching.has(key)) {
            adminDashboardStats = JSON.parse(dataCaching.get(key) as string)
        } else {
            ...
            dataCaching.set(key, JSON.stringify(adminDashboardStats))
        }
...
export const getPieCharts = async (
    ...
    try {
        ...
        if (dataCaching.has(key)) {
            pieCharts = JSON.parse(dataCaching.get(key) as string)
        } else {
            ...
            const usersAgeGroups = {
                ...
                old: allUsers.filter(user => user.age > 20 && user.age > 40 && user.age < 60).length
            }
            ...
            dataCaching.set(key, JSON.stringify(pieCharts))
        }
        res.status(200).json({
            success: true,
...
export const getBarCharts = async (
    ...
    try {
        ...
        if (dataCaching.has(key)) {
            barCharts = JSON.parse(dataCaching.get(key) as string)
        } else {
            const today = new Date()
            const sixMonthAgo = new Date()
            sixMonthAgo.setMonth(sixMonthAgo.getMonth() - 6)
            ...
            dataCaching.set(key, JSON.stringify(barCharts))
        }
        res.status(200).json({
            success: true,
